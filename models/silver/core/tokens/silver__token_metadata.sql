...
token_activity_percentiles AS (
    SELECT
        *,
        ROUND(PERCENT_RANK() over (PARTITION BY blockchain
    ORDER BY
        avg_tx), 2) AS tx_pctl,
        ROUND(PERCENT_RANK() over (PARTITION BY blockchain
    ORDER BY
        avg_senders), 2) AS senders_pctl,
        ROUND(PERCENT_RANK() over (PARTITION BY blockchain
    ORDER BY
        consistency), 2) AS consistency_pctl,
        ROUND(PERCENT_RANK() over (PARTITION BY blockchain
    ORDER BY
        life_days), 2) AS longevity_pctl
    FROM
        token_activity_history
),
transfers_today AS (
    SELECT
        d.block_day,
        ...
        d.unique_senders,
        d.amount,
        ROUND(PERCENT_RANK() over (PARTITION BY d.blockchain, d.block_day
    ORDER BY
        tx_count), 2) AS tx_daily_pctl,
        ROUND(PERCENT_RANK() over (PARTITION BY d.blockchain, d.block_day
    ORDER BY
        unique_senders), 2) AS senders_daily_pctl
    FROM
        {{ ref('silver__transfers_summary') }}
        d
...
joined AS (
    ...
    FROM
        ...
        ON t.address = h.address
        AND t.blockchain = h.blockchain
)
SELECT
    *,
    CASE
        WHEN blockchain = 'aleo'
        AND legitimacy_score > 0.7 THEN TRUE
        WHEN blockchain = 'aptos'
        AND legitimacy_score > 0.9 THEN TRUE
        WHEN blockchain = 'arbitrum'
        AND legitimacy_score > 0.9 THEN TRUE
        WHEN blockchain = 'avalanche'
        AND legitimacy_score > 0.9 THEN TRUE
        WHEN blockchain = 'axelar'
        AND legitimacy_score > 0.8 THEN TRUE
        WHEN blockchain = 'base'
        AND legitimacy_score > 0.95 THEN TRUE
        WHEN blockchain = 'blast'
        AND legitimacy_score > 0.8 THEN TRUE
        WHEN blockchain = 'bob'
        AND legitimacy_score > 0.8 THEN TRUE
        WHEN blockchain = 'boba'
        AND legitimacy_score > 0.8 THEN TRUE
        WHEN blockchain = 'bsc'
        AND legitimacy_score > 0.94 THEN TRUE
        WHEN blockchain = 'core'
        AND legitimacy_score > 0.9 THEN TRUE
        WHEN blockchain = 'cosmos'
        AND legitimacy_score > 0.8 THEN TRUE
        WHEN blockchain = 'eclipse'
        AND legitimacy_score > 0.9 THEN TRUE
        WHEN blockchain = 'ethereum'
        AND legitimacy_score > 0.9 THEN TRUE
        WHEN blockchain = 'flow'
        AND legitimacy_score > 0.8 THEN TRUE
        WHEN blockchain = 'gnosis'
        AND legitimacy_score > 0.93 THEN TRUE
        WHEN blockchain = 'ink'
        AND legitimacy_score > 0.9 THEN TRUE
        WHEN blockchain = 'kaia'
        AND legitimacy_score > 0.9 THEN TRUE
        WHEN blockchain = 'mantle'
        AND legitimacy_score > 0.8 THEN TRUE
        WHEN blockchain = 'maya'
        AND legitimacy_score > 0.8 THEN TRUE
        WHEN blockchain = 'near'
        AND legitimacy_score > 0.8 THEN TRUE
        WHEN blockchain = 'optimism'
        AND legitimacy_score > 0.9 THEN TRUE
        WHEN blockchain = 'osmosis'
        AND legitimacy_score > 0.8 THEN TRUE
        WHEN blockchain = 'polygon'
        AND legitimacy_score > 0.94 THEN TRUE
        WHEN blockchain = 'ronin'
        AND legitimacy_score > 0.9 THEN TRUE
        WHEN blockchain = 'sei'
        AND legitimacy_score > 0.9 THEN TRUE
        WHEN blockchain = 'solana'
        AND legitimacy_score > 0.90 THEN TRUE
        WHEN blockchain = 'stellar'
        AND legitimacy_score > 0.95 THEN TRUE
        WHEN blockchain = 'swell'
        AND legitimacy_score > 0.90 THEN TRUE
        WHEN blockchain = 'thorchain'
        AND legitimacy_score > 0.90 THEN TRUE
        WHEN blockchain = 'ton'
        AND legitimacy_score > 0.90 THEN TRUE
        ELSE FALSE
