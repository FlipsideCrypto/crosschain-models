...
WITH raw AS (
    ...
    SELECT
        livequery.live.udf_api('https://raw.githubusercontent.com/fixes-world/token-list-jsons/refs/heads/main/jsons/mainnet/flow/default.json') AS rawoutput
),
format_output AS (
    SELECT
...
put_together AS (
    ...
    FROM
        format_output
    WHERE
        project_name NOT LIKE 'EVMVMBridgedToken%'
)
SELECT
    ...
    {{ dbt_utils.generate_surrogate_key(['address']) }} AS flow_evm_token_labels_id,
