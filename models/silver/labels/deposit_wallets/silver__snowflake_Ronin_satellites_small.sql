{{ config(
    ...
    post_hook = "delete from {{this}} a using {{ ref('silver__snowflake_Ronin_satellites') }} b where a.address = b.address ",
...
SELECT
    ...
    {{ dbt_utils.generate_surrogate_key(['f.address']) }} AS snowflake_ronin_satellites_id,
    '{{ invocation_id }}' AS _invocation_id
FROM
    final_base f
    LEFT JOIN {{ ref('silver__address_labels') }} A
    ON f.address = A.address
    AND A.blockchain = 'ronin'
