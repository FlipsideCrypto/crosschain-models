CREATE OR REPLACE SCHEMA deposit_wallet_sol; 
USE SCHEMA deposit_wallet_sol; 
CREATE OR REPLACE TABLE sol_addr_labels LIKE bronze.prod_address_label_sink_291098491;  
CREATE OR REPLACE TABLE sol_address_labels_staging (
...
INSERT INTO sol_address_labels_staging (
  ...
  possible_sats AS (
    ...
    from (
      ...
      count(distinct project_name) over(partition by dc.blockchain, xfer.tx_from) as project_count -- how many projects has each from address sent to
      FROM solana.core.fact_transfers xfer
      JOIN distributor_cex dc ON dc.address = xfer.tx_to
   ...
   real_sats as (
     ...
     FROM solana.core.fact_transfers xfer
     LEFT OUTER JOIN distributor_cex dc ON dc.address = xfer.tx_to
     WHERE block_timestamp > current_date - 10
     AND tx_from IN (select address from possible_sats)
     AND amount > 0
     GROUP BY tx_from
   ),
   exclusive_sats AS (
    SELECT tx_from as address FROM real_sats WHERE project_count = 1 GROUP BY 1
  )
...
DELETE FROM sol_address_labels_staging l  
...
SELECT * FROM deposit_wallet_sol.sol_address_labels_staging