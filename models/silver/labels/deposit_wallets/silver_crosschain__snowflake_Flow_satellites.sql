CREATE OR REPLACE SCHEMA deposit_wallet_flow; 
USE SCHEMA deposit_wallet_flow; 
CREATE OR REPLACE TABLE flow_addr_labels LIKE bronze.prod_address_label_sink_291098491;  
CREATE OR REPLACE TABLE flow_address_labels_staging (
...
INSERT INTO flow_address_labels_staging (
...
WITH distributor_cex AS (
  ...
  possible_sats AS (
    ...
    from (
      ...
      count(distinct project_name) over(partition by dc.blockchain, xfer.sender) as project_count -- how many projects has each from address sent to
      FROM FLOW.CORE.EZ_TOKEN_TRANSFERS xfer
      JOIN distributor_cex dc ON dc.address = xfer.recipient
      ...
      FROM ethereum.core.fact_traces tr
   ...
   real_sats as (
     ...
     FROM ethereum.core.fact_token_transfers xfer
...
DELETE FROM flow_address_labels_staging l  
...
SELECT * FROM deposit_wallet_flow.flow_address_labels_staging