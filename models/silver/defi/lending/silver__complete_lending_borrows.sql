{{ config(
    ...
    post_hook = "ALTER TABLE {{ this }} ADD SEARCH OPTIMIZATION ON EQUALITY(tx_hash, contract_address, token_address, token_symbol, borrower, protocol_market), SUBSTRING(token_address, token_symbol, borrower, protocol_market)",
    tags = ['hourly']
) }}
...
WITH ethereum AS (
    ...
    SELECT
        ...
        {{ dbt_utils.generate_surrogate_key(
            ['ez_lending_borrows_id', 'blockchain']
...
SELECT
    ...
    CASE 
        ...
        WHEN platform = 'Joe Lend' THEN 'joe_lend'
        ELSE lower(split_part(platform, ' ', 1))
    END AS protocol,
    CASE 
        WHEN platform = 'Morpho Blue' THEN 'v1'
        WHEN platform = 'Joe Lend' THEN 'v1'
