    ...
    for (var repo_counter = 0; repo_counter < repos.length; repo_counter++) {
        ...
        for (var endpoint_counter = 0; endpoint_counter < endpoints.length; endpoint_counter++) {
            ...
            var create_temp_table_command = `
                ...
                WITH api_call AS (
                    SELECT 
                        ethereum.streamline.udf_api('GET', '${endpoint_url}', { 'Authorization': 'token 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', 'Accept': 'application/vnd.github.v3+json' },{}) AS res,
